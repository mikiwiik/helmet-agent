...
**Expected behaviour:**
1. Resolve "Munkkiniemi" → Finna building code `2/Helmet/h/h33/`
2. Search Finna: `lookfor=väinö linna`, `type=Author`, `filter[]=building:"2/Helmet/h/h33/"`, `filter[]=format:"0/Book/"`
3. Return list of matching titles with year and Finna link
**Status:** implemented ✅ — tested in `test_tools.py` and `test_integration.py`
...
**Expected behaviour:**
1. Resolve "Kallio" → Kirkanta library via name/shortName search
...
**Expected behaviour:**
1. Search Finna: `lookfor=dinosaurus`, `filter[]=building:"1/Helmet/e/"`, `filter[]=format:"0/Book/"`
2. Return results with branch info
**Status:** implemented ✅ — tested in `test_integration.py`
...
**Expected behaviour:**
1. Search Finna: `lookfor=sinuhe egyptiläinen`, `filter[]=format:"0/Sound/"`, `filter[]=building:"0/Helmet/"`
2. Return matching audio records
**Status:** implemented ✅ — tested in `test_tools.py` (format filter)
